...
### Predefined secret with secret_wo

Application credential below will have only one `swiftoperator` role using `secret_wo`
...
* `secret_wo` - (Optional) The secret for the application credential using the
    write only sematics in combination with `ephemeral` resources.

* `secret_wo_version` - (Optional) The version of the write only secret, update it only if
    you have updated the `secret_wo` or you want to update the secret value on Openstack.
