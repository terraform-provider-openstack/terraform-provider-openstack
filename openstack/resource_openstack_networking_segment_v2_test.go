...
func testAccCheckNetworkingV2SegmentDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := segments.Get(context.TODO(), networkingClient, rs.Primary.ID).Extract()
		if err == nil {
...
func testAccCheckNetworkingV2SegmentExists(n string, segment *segments.Segment) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		found, err := segments.Get(context.TODO(), networkingClient, rs.Primary.ID).Extract()
		if err != nil {
			return err
		}
